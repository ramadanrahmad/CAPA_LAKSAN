  ...
  const fetchStats = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        const masuk = data.filter(d => d.status === 'Menunggu Persetujuan').length;
        const revisi = data.filter(d => d.status === 'Perlu Evaluasi').length;
        const selesai = data.filter(d => d.status === 'Sertifikasi Selesai').length;
      ...
      <aside className="dashboard-sidebar">
        ...
        <nav className="sidebar-menu">
          <NavLink 
            ...
            className={() => `menu-item ${location.pathname === '/dashboard' || (location.pathname.includes('/sertifikasi/capa') && (!location.pathname.endsWith('/pdf') && !location.pathname.endsWith('/view') || location.search.includes('from=antrean'))) ? 'active' : ''}`}
          >
