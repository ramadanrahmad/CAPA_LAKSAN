...
export default function SertifikasiSupervisor() {
  const navigate = useNavigate();
  const [permohonan, setPermohonan] = useState([]);
  const [allSertifikasi, setAllSertifikasi] = useState([]);
  ...
  const fetchData = async () => {
    try {
      ...
      if (res.data.success) {
        const sertifikasiOnly = res.data.data.filter(item => item.type === 'sertifikasi');
        setAllSertifikasi(sertifikasiOnly);
        const selesaiOnly = sertifikasiOnly.filter(item => item.status === 'Sertifikasi Selesai');
  ...
  const antreanMasukCount = allSertifikasi.filter(d => d.status === 'Menunggu Persetujuan' || d.status === 'Menunggu Penjadwalan').length;
  const perluEvaluasiCount = allSertifikasi.filter(d => d.status === 'Perlu Evaluasi' || d.status === 'Menunggu Evaluasi' || d.status === 'Upload File').length;
  ...
  return (
    <div className="sertifikasi-selesai-page">
      ...
      <div className="permohonan-header" style={{ marginBottom: '20px' }}>
        <h2 style={{ margin: 0, color: 'var(--text-primary)' }}>Riwayat Sertifikasi Selesai</h2>
      </div>
          ...
          <table className="bpom-table">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan="9" style={{ textAlign: 'center', padding: '20px' }}>Belum ada sertifikasi yang selesai.</td>
                </tr>
              ) : (
