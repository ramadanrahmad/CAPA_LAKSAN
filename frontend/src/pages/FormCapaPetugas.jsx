...
export default function FormCapaPetugas() {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (isReadOnly || isEditMode) {
      ...
      // New Form
      setRows([
        { id: Date.now(), temuan: '', kriteria: 'Mayor', persyaratan: '', gap_analysis: '', dampak: '', tindakan_perbaikan: '', tindakan_pencegahan: '', waktu_penyelesaian: '', pic: '', bukti: '', status_capa: 'Open', hasil_evaluasi: '' }
      ]);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const allClosed = rows.every(r => r.status_capa === 'Closed');
      const targetStatus = allClosed ? 'Sertifikasi Selesai' : 'Menunggu Evaluasi';
      ...
      let res;
      if (isEditMode) {
        res = await sertifikasiAPI.updateCapa(id, payload);
      } else {
        payload.permohonan_id = id;
        res = await sertifikasiAPI.createCapa(payload);
      }

      if (res.data.success) {
        toast.success(isEditMode ? 'Evaluasi CAPA berhasil disimpan!' : 'Temuan CAPA berhasil dikirim!');
        navigate('/dashboard');
      }
    } catch (error) {
              ...
              {rows.map((row, rowIndex) => {
                ...
                return (
                  <tr key={row.id} style={{
                    ...
                    {/* Evaluasi & Status */}
                    {(!isEditMode && !isReadOnly) ? (
                      <td colSpan="2" style={{ background: 'rgba(0,0,0,0.02)' }}></td>
                    ) : (
                      <>
                        <td style={{ position: 'relative' }}>
                          {showLockPetugas && <LockIcon />}
                        ...
                        <td style={{ position: 'relative' }}>
                          ...
                          <select
                            className="glass-input select-input"
                            value={row.status_capa}
                            onChange={(e) => handleChange(rowIndex, 'status_capa', e.target.value)}
                            style={{ width: '100%', padding: '12px 4px', fontWeight: 'bold', color: row.status_capa === 'Closed' ? '#10b981' : '#ef4444' }}
                            disabled={disableEvaluasiFields}
                          >
          ...
          <div style={{ display: 'flex', gap: '15px' }}>
            ...
            {!isReadOnly && (
              <button
                ...
                {isSubmitting ? 'Memproses...' : (isEditMode ? 'Kirim Evaluasi' : 'Kirim Temuan')}
              </button>
            )}
          </div>
        </div>
