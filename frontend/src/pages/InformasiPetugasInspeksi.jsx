...
export default function InformasiPetugasInspeksi() {
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const res = await inspeksiAPI.getInformasiPetugas();
      if (res.data.success) {
  ...
  return (
    <div className="sertifikasi-page">
      <div className="permohonan-header" style={{ marginBottom: '20px' }}>
        <h2 style={{ margin: 0, color: 'var(--text-primary)' }}>Informasi CAPA Inspeksi</h2>
      </div>
