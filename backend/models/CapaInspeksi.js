...
const CapaInspeksi = {
  ...
  async createBulk(rows) {
    ...
    const promises = rows.map(row => {
      return pool.query(
        `INSERT INTO capa_inspeksi (permohonan_id, temuan, kriteria, persyaratan) 
         VALUES (?, ?, ?, ?)`,
        [
          row.permohonan_id,
          row.temuan,
  ...
  async findByPermohonanId(permohonan_id) {
    const [rows] = await pool.query(
      `SELECT * FROM capa_inspeksi WHERE permohonan_id = ? ORDER BY id ASC`,
      [permohonan_id]
    );
    return rows;
  },
  ...
  async snapshotHistory(permohonan_id, tahap) {
    const [result] = await pool.query(
      `INSERT INTO capa_inspeksi_history 
        (capa_id, permohonan_id, tahap, temuan, kriteria, persyaratan, gap_analysis, dampak, 
         tindakan_perbaikan, tindakan_pencegahan, waktu_penyelesaian, pic, bukti, file_surat, hasil_evaluasi, status_capa)
  ...
  async findHistoryByPermohonanId(permohonan_id) {
    const [rows] = await pool.query(
      `SELECT * FROM capa_inspeksi_history WHERE permohonan_id = ? ORDER BY tahap ASC, capa_id ASC`,
      [permohonan_id]
    );
    return rows;
  }
